...
/**
 * Bitcoin data processing service implementation.
 */
public class BitcoinDataProcessingService implements DataProcessingService {
    
    private final BitcoinConfiguration config;
    private final PluginContext context;
    private final AtomicBoolean healthy = new AtomicBoolean(false);
    
    public BitcoinDataProcessingService(BitcoinConfiguration config, PluginContext context) {
        this.config = config;
    ...
    public Mono<List<BlockchainEvent>> extractEvents(Block block, List<EventFilter> eventFilters) {
        return Mono.just(List.of()); // Bitcoin doesn't have events
    }
